...
| 09 | Sokoban | `TileMap` (niveles = tilemaps) | **Jugable en `examples/games/12_sokoban.c`.** Valida `TileMap` como fuente de verdad de un nivel cuyas celdas cambian de rol (empujar cajas), el patrón `TGE_Playfield` sin `fixedstep` (movimiento event-driven por tecla) y un undo específico del juego. Lógica mínima: grid + input + undo, sin módulos nuevos. |
| 10 | Minesweeper | Mouse (botones, click derecho) + flood fill + RNG con seed | **Jugable en `examples/games/13_minesweeper.c`.** Primer juego que exige el mouse en serio (revelar con clic izquierdo, marcar con clic derecho) sobre la capa de eventos `TGE_EVENT_MOUSE*`. Valida el flood fill BFS sobre el grid (cola acotada con marca `queued`), la generación determinista con seed (xorshift32 + Fisher-Yates) y el primer clic seguro (mueve la mina). Fallback de teclado incluido. Pruebas en `tests/test_minesweeper.c`. |
