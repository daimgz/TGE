...
static void draw_body(TGE_EntityPool *pool, TGE_EntityId id, void *userdata, void *ctx);
static void game_reset(Game *g);
static void game_free(Game *g);
static void free_body(TGE_EntityPool *pool, TGE_EntityId id, void *userdata, void *ctx);
static void start_wave(Game *g);
static bool spawn_enemy(Game *g);
static bool spawn_bullet(Game *g, int x, int y);
static void update_body(TGE_EntityPool *pool, TGE_EntityId id, void *userdata, void *ctx);
static void collide_pass(Game *g);
static void bullet_collide(TGE_EntityPool *pool, TGE_EntityId id, void *userdata, void *ctx);
static void breach_check(TGE_EntityPool *pool, TGE_EntityId id, void *userdata, void *ctx);
...
static void game_draw(TGE_Scene *scene, TGE_Canvas *canvas)
{
    ...
    tge_printf(canvas, 1, 0, TGE_COLOR_YELLOW, TGE_COLOR_DEFAULT,
               " SCORE %06d ", g->score);
    tge_printf(canvas, 17, 0, TGE_COLOR_CYAN, TGE_COLOR_DEFAULT, " LV %d ",
               g->level);
    tge_printf(canvas, 25, 0, TGE_COLOR_GREEN, TGE_COLOR_DEFAULT, " LIVES %d ",
               g->lives);
    tge_printf(canvas, 37, 0, TGE_COLOR_WHITE, TGE_COLOR_DEFAULT, " LEFT %02d ",
               g->live);
