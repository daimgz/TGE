...
int tge_utf8_decode(const char *data, int size, uint32_t *out_codepoint)
{
    ...
    if (b0 <= 0xEF) {
        if (size < 3)
            return -1;
        unsigned char b1 = (unsigned char)data[1];
        unsigned char b2 = (unsigned char)data[2];
    ...
    if (b0 <= 0xF4) {
        ...
        unsigned char b1 = (unsigned char)data[1];
        unsigned char b2 = (unsigned char)data[2];
        unsigned char b3 = (unsigned char)data[3];
...
static int in_range(uint32_t cp, const uint32_t *ranges, int count)
{
...
int tge_utf8_char_width(uint32_t codepoint)
{
    ...
    if (in_range(codepoint, combining, NELEMS(combining)))
        return 0;

    if (in_range(codepoint, wide, NELEMS(wide)))
        return 2;
